...
import { TokenRustDapp } from "../target/types/token_rust_dapp"
...
async function createTokenAccount(provider, mint, owner) {
  ...
  const tx = new anchor.web3.Transaction();
  tx.add(
    ...(await createTokenAccountInstrs(provider, vault.publicKey, mint, owner, 12))
  );
...
describe("token-rust-dapp", () => {
  // Configure the client to use the local cluster.
  const provider = anchor.Provider.env();
  anchor.setProvider(provider);

  const program = anchor.workspace.TokenRustDapp as Program<TokenRustDapp>;
